...
- Non ci devono essere nei caratteri le parentesi tonde
NON PRESTARE ATTENZIONE A UNA DELLE SEGUENTI CONDIZIONI DI MODIFICA PUO' PORTARE A ERRORI O CALCOLI MAL ESEGUITI!
NEL CODICE NON C'E' NESSUN CONTROLLO SU QUESTE CONDIZIONI (ANCHE PER MOTIVI DI PRESTAZIONI)
...
bool findInVector(const vector<string> &v,string s){
...
int findPosInVector(const vector<string> &v,string s){
	for(int i=0;i<v.size();i++){
		if (v[i] == s){
			return i;
		}
...
bool CalculateExpression(const vector<string> &signTable,bool* signValues,string ex){
	char here;
